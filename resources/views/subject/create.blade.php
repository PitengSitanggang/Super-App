...
<div class="max-w-xl w-full">
        <div class="mb-6 flex items-center justify-between">
            <h1 class="text-3xl font-bold text-gray-800">Add New Subject</h1>
            ...
            <form action="{{ route('subjects.store') }}" method="POST">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code') }}" required placeholder="e.g. MATH-101"
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Advanced Mathematics"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
                        @error('name')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="group" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition bg-white">
                            ...
                            <option value="Arts" {{ old('group') == 'Arts' ? 'selected' : '' }}>Arts</option>
                            <option value="Language" {{ old('group') == 'Language' ? 'selected' : '' }}>Language</option>
                            <option value="Physical Education" {{ old('group') == 'Physical Education' ? 'selected' : '' }}>Physical Education</option>
                            <option value="Technology" {{ old('group') == 'Technology' ? 'selected' : '' }}>Technology</option>
                        </select>
                        @error('group')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
