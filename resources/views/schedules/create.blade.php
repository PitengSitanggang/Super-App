...
@section('header_title', 'Create Schedule')
...
<div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
    <form action="{{ route('schedules.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="class_name" value="{{ old('class_name') }}" required class="w-full rounded-xl border-gray-200 shadow-sm focus:border-primary focus:ring-primary px-4 py-3 bg-gray-50/50">
                ...
                <div class="flex flex-wrap gap-4 mt-2">
                    @for($i = 1; $i <= 7; $i++)
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="period[]" value="{{ $i }}" class="rounded border-gray-300 text-primary focus:ring-primary h-5 w-5" {{ in_array($i, old('period', [])) ? 'checked' : '' }}>
